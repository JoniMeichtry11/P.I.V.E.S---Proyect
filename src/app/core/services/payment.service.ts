...
export class PaymentService {
  ...
  async createPreference(
    ...
    const preferenceBody: any = {
      items: [
        {
          ...
          description:
            "Combustible para reservar autos en la plataforma P.I.V.E.S.",
          quantity: 1,
          currency_id: "ARS",
      ...
      back_urls: {
        success: `${appUrl}/buy-fuel/status?result=approved&liters=${pkg.liters}&childId=${childId}`,
        failure: `${appUrl}/buy-fuel/status?result=failure`,
        pending: `${appUrl}/buy-fuel/status?result=pending&liters=${pkg.liters}&childId=${childId}`,
      },
      metadata: {
        user_id: user.uid,
        child_id: childId,
