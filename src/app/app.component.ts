...
@Component({
  ...
  template: `
    <div class="min-h-screen flex flex-col bg-sky-50 text-slate-800">
      ...
      <main
        [class.pt-16]="showHeader$ | async"
        class="flex-1 transition-opacity duration-500 ease-in-out"
      >
...
export class AppComponent implements OnInit, OnDestroy {
    ...
    this.isAdmin$ = this.userService.currentUserAccount$.pipe(
      map(
        (account) =>
          ...
          account?.parent?.email === SUPER_ADMIN_EMAIL,
      ),
      startWith(false),
    );
  }
  ...
  ngOnInit(): void {
    // Check for Firebase Auth Action Codes (email verification, etc.)
    const urlParams = new URLSearchParams(window.location.search);
    const mode = urlParams.get("mode");
    const oobCode = urlParams.get("oobCode");

    if (mode === "verifyEmail" && oobCode) {
      this.router.navigate(["/login"], { queryParams: { mode, oobCode } });
      return;
    }
